...
export default function ProjectManagement() {
  ...
  const loadWorkerDetail = async (workerId) => {
    ...
    const [credRes, darRes] = await Promise.all([
      ...
      api.get(`/projects/${selected.id}/workers/${workerId}/dar-status`),
    ]);
    setWorkerDetail({
      workerId,
                ...
                {workerDetail && (
                  <div className="mt-4 p-4 bg-blue-50 rounded-xl">
                    <div className="flex items-center justify-between mb-3">
                      <h4 className="font-medium text-blue-900">Professional profile (credentials &amp; DAR)</h4>
                    ...
                    {workerDetail.darError && (
                      <p className="text-xs text-amber-800 bg-amber-50 border border-amber-100 rounded-lg px-3 py-2 mb-3">{workerDetail.darError}</p>
                    )}
                    {workerDetail.darStatus && (
                      <div className="rounded-xl border border-indigo-200 bg-white/80 p-3 mb-3 space-y-2">
                        <h5 className="text-sm font-semibold text-gray-900">Data access requirements (DAR)</h5>
                        <p className="text-xs text-gray-600">
                          ...
                          {workerDetail.darStatus.issued_count > 0 && (
                            <span className="text-indigo-700"> · {workerDetail.darStatus.issued_count} issued (awaiting)</span>
                          )}
                        </p>
                        {workerDetail.darStatus.dar_requested_at && (
                          <p className="text-xs text-gray-500">DAR notification: {workerDetail.darStatus.dar_requested_at}</p>
                        )}
                        <ul className="space-y-1">
                          {(workerDetail.darStatus.requirements || []).map((r) => (
                            <li key={r.id} className="flex justify-between gap-2 text-xs p-2 rounded-lg bg-gray-50">
                              <span>{r.label}</span>
                              <span className={
                                r.status === 'satisfied' ? 'text-green-700 font-medium' :
                                r.status === 'issued' ? 'text-amber-700 font-medium' : 'text-gray-500'
                              }>
                                {r.status === 'satisfied' ? 'Satisfied' : r.status === 'issued' ? 'Issued' : 'Not issued'}
                              </span>
                            </li>
                          ))}
                        </ul>
