...
function IssueDARModal({ panel, onClose, onIssued, showToast }) {
  ...
  const handleSelectAll = () =>
    setSelected(selected.size === notIssued.length ? new Set() : new Set(notIssued.map(r => r.id)));

  const handleIssue = async () => {
    if (selected.size === 0) return;
    setLoading(true);
    const results = await Promise.all([...selected].map(reqId => issueOne(workerId, reqId)));
    setLoading(false);
    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-3">
          <p className="text-sm text-gray-600">Select requirements to issue to this professional. Once issued they appear in the Credentis Professional app for the professional to respond to.</p>
          {notIssued.length > 0 && (
            <button onClick={handleSelectAll} className="text-xs text-indigo-600 hover:underline">
            ...
            {requirements.map(r => (
              ...
              >
                <div className="flex items-center gap-3">
                  {r.status === 'not_issued' ? (
                    <input type="checkbox" checked={selected.has(r.id)} onChange={() => toggle(r.id)} onClick={e => e.stopPropagation()} className="w-4 h-4 rounded accent-indigo-600" />
...
export default function SubWorkerManagement() {
  ...
  const openIssueDARModal = async (workerId, workerName) => {
    ...
    setIssueDARPanel({
      ...
      issueOne: (wId, rId) =>
        api.post(`/projects/${selectedProject}/workers/${wId}/dar/${rId}/issue`),
    });
  };
  ...
  const handleRequestDar = async (assignmentId) => {
    const res = await api.post(`/projects/${selectedProject}/assignments/${assignmentId}/request-dar`);
    if (res.success) loadWorkers(selectedProject);
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-500 text-sm mt-1">Manage, verify, and endorse professionals on your delegated projects</p>
      </div>
      ...
      <p className="text-sm text-amber-700 flex items-center gap-2">
        ...
        Your actions are visible up the chain to Contractor and Client. You can issue DAR to a professional (send icon); document and eye icons open details in a dialog.
      </p>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-100">
                ...
                <th className="text-left py-3 px-4 font-medium text-gray-500">Issues (DAR / credentials)</th>
            ...
            <tbody>
              {visibleWorkers.map(w => {
                ...
                <tr key={w.id} className="border-b border-gray-50 hover:bg-gray-50">
                  ...
                  <td className="py-3 px-4 text-xs text-gray-600 max-w-[220px]">{Array.isArray(w.issues) ? w.issues.join('; ') : (w.issues || '—')}</td>
                  <td className="py-3 px-4">
                    <select
                      defaultValue=""
                      onChange={(e) => {
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-3">
              ...
              <p className="text-xs text-gray-500">
                Click <strong>Issue to Professional</strong> on any unissued requirement to send it to the professional&apos;s Credentis app.
              </p>
              <div className="space-y-2">
      ...
      {/* Toast */}
      {toast && (
        <div className={`fixed bottom-6 right-6 z-[60] flex items-center gap-3 px-4 py-3 rounded-xl shadow-lg text-white text-sm font-medium ${toast.type === 'error' ? 'bg-red-600' : 'bg-green-600'}`}>
      ...
      {workerDetail && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/40 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-6 border-b border-gray-100">
              <h3 className="font-semibold text-gray-900">Professional &amp; Academic Credentials (VCs)</h3>
              <button onClick={() => setWorkerDetail(null)} className="p-2 rounded-lg hover:bg-gray-100 text-gray-500"><X className="w-5 h-5" /></button>
            </div>
            <div className="p-6 space-y-3">
              <p className="text-sm text-gray-600">As the professional adds credentials to satisfy your DAR they become Verifiable Credentials and are visible up the chain to Contractor and Client.</p>
